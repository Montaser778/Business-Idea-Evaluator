...
from langgraph.checkpoint.sqlite import SqliteSaver # التعديل هنا لحفظ البيانات دائمًا
...
# 4. بناء الـ Graph مع استخدام SqliteSaver للحفظ الدائم
@st.cache_resource
def get_graph():
...
if prompt := st.chat_input("أدخل فكرتك التجارية - المحرك جاهز..."):
    with st.status("🚀 جاري تهيئة المحركات...", expanded=True) as status:
        ...
        st.write("🧠 الوكلاء في مرحلة المعالجة...")
...
if final := graph.get_state(config).values.get("final_report"):
    ...
    st.info(final)import streamlit as st
import operator
import sqlite3
from typing import List, Annotated, Dict
from typing_extensions import TypedDict
